...
Cyton.prototype.connect = function (portName) {
  return new Promise((resolve, reject) => {
    ...
    this.serial.on('data', data => {
      // logs start time
      currentSampleTime = process.hrtime()[0] * 1000 + process.hrtime()[1] / 1000000;
...
Cyton.prototype._finalizeNewSample = function (sampleObject) {
  ...
  // adds start time - end time to total time, then divides by number of samples and prints that
  currentSampleTime = process.hrtime()[0] * 1000 + process.hrtime()[1] / 1000000 - currentSampleTime;
  totalTime += currentSampleTime;
  numberOfSamples++;
  console.log(totalTime / numberOfSamples + " ms/sample");
};
